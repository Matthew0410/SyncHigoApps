  ...
  $arr_login_lookup = array();
	...
	foreach ($arr_higo_router as $higo_router)
	{
	  ...
	  foreach ($arr_date as $date)
	  {
			...
			$count_date_rate5 += (isset($arr_log_lookup[$higo_router->id][date('Y-m-d', $date)])) ? 1 : 0;
	  ...
	  $phpexcel->getActiveSheet()->getStyle("H{$row}")->getNumberFormat()->applyFromArray(array(
		...
		if ($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("J{$row}", round($count_rate4 / $count_date_rate4) / 100);
			$phpexcel->getActiveSheet()->getStyle("J{$row}")->getNumberFormat()->applyFromArray(array(
		    'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
		  ));
		}
		...
		if ($count_login != '') {
			$phpexcel->getActiveSheet()->SetCellValue("K{$row}", round($count_M0Mac / $count_date_rate5, 2));
		}
